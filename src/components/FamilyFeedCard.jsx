...
import { Activity, CheckCircle2, AlertTriangle, CalendarClock, Heart } from 'lucide-react';
...
const STYLES = {
  EMERGENCY: { icon: AlertTriangle, bg: '#FEF2F2', border: '#DC2626', fg: '#991B1B', label: 'CẤP CỨU' },
  SAFETY_CRITICAL: { icon: CalendarClock, bg: '#FFFBEB', border: '#F59E0B', fg: '#B45309', label: 'CẦN ĐI KHÁM' },
  DOSE_TAKEN: { icon: CheckCircle2, bg: '#F0FDF4', border: '#86EFAC', fg: '#166534', label: 'ĐÃ UỐNG' },
  STATUS_OK: { icon: Heart, bg: '#FFF1F2', border: '#FECDD3', fg: '#BE123C', label: 'BÁO TIN' }
};
              ...
              <div key={item.id} style={{ padding: 14, borderRadius: 14, background: s.bg, border: `1px solid ${s.border}`, display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                    <span style={{ fontSize: 10.5, fontWeight: 800, padding: '2px 8px', borderRadius: 99, background: s.border, color: s.fg === '#166534' ? '#166534' : '#FFF' }}>
                      {s.label}
                    </span>
